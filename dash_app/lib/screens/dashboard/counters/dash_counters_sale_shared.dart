...
class SaleSharedCount extends StatelessWidget {
  SaleSharedCount(this.uData);
  final UserData uData;

  @override
  Widget build(BuildContext context) {
    //final user = Provider.of<FirebaseUser>(context);
    return Provider<SaleSharedCountViewModel>(
      create: (_) => SaleSharedCountViewModel(uData: uData),
      child: SalesSharedData(),
    );
  }
}

class SaleSharedCountViewModel {
  SaleSharedCountViewModel({this.uData});
  final UserData uData;
  CollectionReference users = FirebaseFirestore.instance.collection('users');

  Stream<List<SaleItem>> saleSharedStream() {
    ...
    return Rx.combineLatest(send.toList(), (List<QuerySnapshot> values) {
      ...
      finalitem.removeWhere((item) => item.type != '1');
      return finalitem;
    });
  }
}

class SalesSharedData extends StatelessWidget {
  //LeadBuyerCustomersData(this.userId);
  //final String userId;
  @override
  Widget build(BuildContext context) {
    final viewModel =
        Provider.of<SaleSharedCountViewModel>(context, listen: false);
    return StreamBuilder<List<SaleItem>>(
        stream: viewModel.saleSharedStream(),
